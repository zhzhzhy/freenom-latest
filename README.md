...
[⛵  通过 Docker 方式部署](#--方式一：通过-Docker-部署（最简单的方式）)（最简单的方式，推荐）

[🚧  直接拉取源码部署](#--方式二：直接拉取源码部署)
 ...
 **注意：不推荐自定义脚本执行时间。因为你可能跟很多人定义的是同一个时间点，这样可能导致所有人都是同一时间向 Freenom 的服务器发起请求，
 使得 Freenom 无法稳定提供服务。而如果你不自定义时间，程序会自动指定一个北京时间 06 ~ 23 点全时段随机的一个时间点，
 每次重启容器都会自动重新指定。**
...
而`.env`则是配置文件，现在直接执行`vim .env`将`.env`文件里的所有配置项改为你自己的即可。然后重启容器，如果配置正确的话，便很快可以收到相关邮件。
...
| Vultr | [https://www.vultr.com/?ref=7429134](https://www.vultr.com/?ref=7429134) | 随开随停，按小时计费，用多少算多少，我平时需要干净的 VPS 测试时会用它们家的。 |
| PacificRack | [https://pacificrack.com/portal/aff.php?aff=1150](https://pacificrack.com/portal/aff.php?aff=1150) | 低价屠夫，最便宜的机型`10刀`一年，我用了两年了目前感觉很稳。预算有限的情况下选它们家准没错。 |
